		...
		for i in range(len(data)):
			...
			if line.startswith("@") :
				...
				if len(obsdat) >= 3:
					...
					self.__dict__['FLUXERR'].append( [0] )
				if len(obsdat) >= 4:
					self.__dict__['VALID'].append( str2num(obsdat[3]) )
				else:
					self.__dict__['VALID'].append([1])
		for col in colnames : 
			self.__dict__[col] = array( self.__dict__[col] )
		return( None )

	def readfitsspecfile(self, fitsfile ):
		""" read spectrum from a JLA file.
		Metadata in the header are in "@ value"
		column names are given with "# value", will add manually 
		Observation data lines are the rest 
...
"""%len(self.SPECTRA.keys()),file=fout)
	...
	def appendspec2snanafile(self, outfile, specdir, verbose=False, **kwarg ):
		...
		fout = open(outfile,'w')
		list_snana_headers=['SURVEY','SNID','RA','DEC','MWEBV','REDSHIFT_HELIO','MJDPK','SEARCH_PEAKMJD']
		list_jla_headers=['SURVEY','SN','RA','DEC','MWEBV','Z_HELIO','DayMax']
		non_header_keys = ['NOBS','NVAR','MJD','FLT','FIELD','FLUXCAL',
						   'FLUXCALERR','MAG','MAGERR','datfile','BAND',
						   'ZPFLUX','PSF','SKYSIG','GAIN','PHOTFLAG','PHOTPROB','SPECTRA',
						   'Zpt','SNR','EPOCH','PSF_SIG1','PSF_SIG2','PSF_RATIO','FLUX',
						   'FLUX_ERRTOT','FLUXCAL_ERRTOT','MAG_ERRPLUS','ZEROPT','ZEROPT_ERR',
						   'SKYSUB_ERR','GALSUB_ERR','END_OF_EPOCH','END_OF_SN','XPIXEL','YPIXEL',
						   'SEARCH_MAG','SEARCH_MAGERR','PEAKMJD','NEPOCH','REDSHIFT_Status']
		if 'SEARCH_PEAKMJD' not in self.__dict__.keys() and 'PEAKMJD' in self.__dict__.keys():
			self.SEARCH_PEAKMJD = self.PEAKMJD
		...
		if self.SURVEY == 'SDSS': self.SNID = '%06i'%self.SNID
		elif type(self.SNID) == float: self.SNID = str(int(self.SNID))
		list_file_spec=glob.glob(specdir+'/spectrum*'+str(self.__dict__['SNID'])+'*.list')
		if not len(list_file_spec):
			list_file_spec=glob.glob('%s/%s/*%s*flm'%(specdir,str(self.__dict__['SNID']),str(self.__dict__['SNID'])))
			if not len(list_file_spec):
				return( None )
		...
		for specfile in sorted(list_file_spec):
			...
			print('SPECTRUM_NLAM: %i'%len(sn_spectrum.WAVE),file=fout)
